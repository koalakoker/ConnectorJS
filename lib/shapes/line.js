class Line extends Shape {
  ...
  updateProperties() {
    super.updateProperties((html) => {
      html = replaceTokens(html, 1, this.start.x);
      html = replaceTokens(html, 2, this.start.y);
      html = replaceTokens(html, 3, this.center.x);
      html = replaceTokens(html, 4, this.center.y);
      html = replaceTokens(html, 5, this.end.x);
      html = replaceTokens(html, 6, this.end.y);
      return html;
    });
  }
  valueChanged(id, value) {
    if (id === "center.x") {
      ...
      InputManager.updateNested(this, id, value, true);
      this.center = midPoint(this.start, this.end);
    }
    return false;
  }
}
