class Line extends Shape {
  ...
  draw(parentCenter = new Point(), parentPivot = new Point()) {
    let start = translate(this.start, parentCenter);
    start = translate(start, parentPivot.neg());
    let end = translate(this.end, parentCenter);
    end = translate(end, parentPivot.neg());
    drawLine(start, end, this.pen);
    super.draw();
  }
  isOnItem(p) {
